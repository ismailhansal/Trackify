...
export default function AddClient() {
  ...
  // Fonction pour récupérer les crédits existants depuis Firestore
  useEffect(() => {
    const fetchClients = async () => {
      const q = query(collection(db, "Credit"), orderBy("DueDate"));
      const querySnapshot = await getDocs(q);
      const clientsList = [];
      querySnapshot.forEach((doc) => {
        clientsList.push({
          id: doc.id, // Ajouter l'ID du document pour référence
          ...doc.data(),
        });
      });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError("Error while adding the credit: " + error.message); // Ajout du message d'erreur détaillé
    }
  };

  // Fonction pour supprimer un crédit payé de Firestore et ajouter l'élément dans la table "Items" avec le champ "price"
  const handlePaymentConfirmed = async (id, amountPaid) => {
    try {
      // Supprimer le document de Firestore
      await deleteDoc(doc(db, "Credit", id));

      // Ajouter un paiement dans la collection "Items" avec le champ "price"
      await addDoc(collection(db, "items"), {
        price: amountPaid,  // Enregistrer le montant payé dans le champ "price"
      });
    ...
    <div className="flex">
      ...
      <div className="sm:ml-64 p-6 flex-1">
        <div className="flex flex-col md:flex-row items-center justify-center md:gap-2 rounded-lg bg-gray-800 py-4">
          <h1 className="text-white text-center text-xl font-medium mb-2 md:mb-0">Add a Credit to Receive</h1>
        </div>

        {error && <div className="text-red-500 mb-4">{error}</div>}
        {success && <div className="text-green-500 mb-4">Client added successfully!</div>}

        <form onSubmit={handleSubmit} className="mt-6 bg-[#010101] text-white shadow-md rounded-lg p-8 max-w-4xl mx-auto">
          <div className="mb-6">
            <label htmlFor="clientName" className="block font-semibold text-lg">From</label>
            <input
              type="text"
              id="clientName"
              value={clientName}
      ...
      <div className="sm:ml-4 p-6 w-full md:w-1/3 bg-[#2D2D2D] rounded-lg shadow-md mt-6 md:mt-0 ml-4">
        <h2 className="text-white text-xl font-semibold mb-4">Clients with Due Credits</h2>
        {clients.length === 0 ? (
          <div className="text-gray-300">No clients with due credits yet.</div>
        ) : (
